<x-app-layout>
    ...
    <div>
        ...
        <div id="createTaskModal"
            class="fixed inset-0 bg-[#101010] bg-opacity-75 flex items-center justify-center z-50 hidden transition-opacity duration-300">
            {{-- modal content --}}
            <div
                class="bg-[#1C1C1C] border-[1px] border-[#2e2e2e] rounded-2xl  shadow-md shadow-black/40 w-full max-w-md p-6">
                {{-- modal header --}}
                ...
                <form action="{{ route('team.tasks.store', $team) }}" method="POST" class="mt-4">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" id="description" rows="1"
                            class="block mt-1 w-full border-[#fff]/25 bg-transparent focus:outline-none focus:ring-0 focus:border-[#fff]/25 text-[#fff] rounded-md shadow-sm"></textarea>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <x-text-input id="start" class="block mt-1 w-full" type="datetime-local" name="start"
                            required autocomplete="" min="{{ date('Y-m-d\TH:i') }}" />
                    </div>
                    ...
                    <div>
                        <div class="flex flex-col gap-2">
                            @foreach ($team->users as $user)
                                <div
                                    class="flex items-center justify-between border-[#2e2e2e] border-[1px] py-2 px-3 rounded-md shadow-sm cursor-pointer">
                                    <div class="flex items-center gap-2">
                                        ...
                                        @else
                                            <div
                                                class="w-8 h-8 rounded-full border-2 border-[#2e2e2e] bg-gray-600 flex items-center justify-center">
                                    ...
                                    <div class="">
                                        <a href=""
                                            class="text-xs font-medium text-[#6737f5]/60  hover:text-[#6737f5] underline">Remove</a>
                                    </div>
                                </div>
                            @endforeach
                        </div>
            ...
            @if ($user->image)
                <a href={{ route('profile.edit') }}>
                    <img src="{{ asset('storage/' . $user->image) }}" alt="Profile Picture"
                        class="w-12 h-12 rounded-full object-cover cursor-pointer border-[2px] border-[#2e2e2e]">
                </a>
            @else
                <a href={{ route('profile.edit') }}>
            ...
            <div class="overflow-hidden rounded-xl border-[1px] border-[#2e2e2e]">
                <table class="w-full">
                    <thead>
                        <tr class="bg-[#272727]">
                            <th
                                class="text-left py-4 px-6 text-xs uppercase tracking-wider text-white/50 font-semibold">
                                Task</th>
                                ...
                                <td class="py-4 px-6">
                                    @if ($task->assigned_to)
                                        <div class="flex items-center gap-2">
                                            ...
                                            @else
                                                <div
                                                    class="w-6 h-6 rounded-full border-2 border-[#2e2e2e] bg-gray-600 flex items-center justify-center">
                                                    <span class="text-white text-xs">
                                    ...
                                    <div class="flex  justify-end gap-4">
                                        @if (
                                            $task->status !== 'completed' &&
                                                (Auth::id() === $team->owner_id || Auth::id() === $task->assigned_to || Auth::id() === $task->user_id))
                                            <form action="{{ route('team.tasks.complete', [$team, $task]) }}"
                                                method="POST" class="inline">
                                                @csrf
                                                <button type="submit"
                                                    class="text-white/60 hover:text-green-500 transition-colors">
                                        ...
                                        @if (Auth::id() === $team->owner_id || Auth::id() === $task->user_id)
                                            <button data-task-id="{{ $task->id }}"
                                                onclick="document.getElementById('editTaskTeamModal-{{ $task->id }}').classList.remove('hidden')"
                                            ...
                                            {{-- edit task modal --}}
                                            <div id="editTaskTeamModal-{{ $task->id }}"
                                                class="fixed inset-0 bg-[#101010] bg-opacity-75 flex items-center justify-center z-50 hidden transition-opacity duration-300">
                                                <div
                                                    class="bg-[#1C1C1C] border-[1px] border-[#2e2e2e] rounded-2xl shadow-md shadow-black/40 w-full max-w-md p-6">
                                                    ...
                                                    <div class="flex justify-between items-center pb-4">
                                                        ...
                                                        <button
                                                            class="text-[#eb8541] hover:text-[#fff] text-[25px] focus:outline-none cursor-pointer transition duration-300"
                                                            onclick="document.getElementById('editTaskTeamModal-{{ $task->id }}').classList.add('hidden');">
                                                            &times;
                                                        </button>
                                                    </div>

                                                    {{-- Modal Form --}}
                                                    <form action="{{ route('team.tasks.update', [$team, $task]) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('PUT')
                                                        ...
                                                        <div class="mb-4">
                                                            <x-input-label for="description" :value="__('Task description')" />
                                                            <textarea name="description" id="description" rows="1"
                                                                class="block mt-1 w-full border-[#fff]/25 bg-transparent focus:outline-none focus:ring-0 focus:border-[#fff]/25 text-[#fff] rounded-md shadow-sm">{{ $task->description }}</textarea>
                                                        </div>

                                                        {{-- Task Deadline --}}
                                                        <div class="mb-4">
                                                            <x-input-label for="start" :value="__('Task start')" />
                                                            <x-text-input value="{{ $task->start }}" id="start"
                                                        ...
                                                        <div class="mb-4">
                                                            <x-input-label for="end" :value="__('Task end')" />
                                                            <x-text-input value="{{ $task->end }}" id="end"
                                                                class="block mt-1 w-full" type="datetime-local"
                                                                name="end" required autocomplete=""
                                                                min="{{ date('Y-m-d\TH:i') }}" />
                                                        </div>
                                                        ...
                                                        <div class="mb-4">
                                                            ...
                                                            <select id="priority" name="priority"
                                                                ...
                                                                <option value="low"
                                                                    ...
                                                                    class="text-[#000]">Low</option>
                                                                <option value="medium"
                                                                    {{ $task->priority === 'medium' ? 'selected' : '' }}
                                                                    class="text-[#000]">Medium</option>
                                                                <option value="high"
                                                                    {{ $task->priority === 'high' ? 'selected' : '' }}
                                                                    class="text-[#000]">High</option>
                                                            </select>
                                                        </div>
                                                        ...
                                                        @if (Auth::id() === $team->owner_id)
                                                            <div class="mb-4">
                                                                ...
                                                                <select id="assigned_to" name="assigned_to"
                                                                    class="block mt-1 w-full border-[#fff]/25 bg-transparent focus:outline-none focus:ring-0 focus:border-[#fff]/25 text-[#fff] rounded-md shadow-sm">
                                                                    <option value="" class="text-[#000]">Select
                                                                        team member</option>
                                                                    @foreach ($team->users as $member)
                                        ...
                                        @if (Auth::id() === $team->owner_id || Auth::id() === $task->user_id)
                                            <form action="{{ route('team.tasks.destroy', [$team, $task]) }}"
                                                method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
